...
try:
    from commune import CommuneClient as Commune
except ImportError:
    raise ImportError(
...
class CommuneTools(Toolkit):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__(name="commune", **kwargs)

        # Register email tools
        if enable_email:
            self.register(self.send_email)
            self.register(self.read_inbox)
    ...
    def read_sms(self, limit: int = 10) -> str:
        """List recent inbound and outbound SMS messages.

        Use this tool to view SMS history — for example to check for replies
        to previously sent messages or to review inbound SMS from customers.

        Args:
            limit: Maximum number of messages to return (default ``10``).

        Returns:
            str: A formatted, human-readable list of SMS messages.  Each entry
            shows direction (FROM/TO), phone number, relative timestamp, and
            message body preview.  Returns ``"No SMS messages found."`` when
    ...
    def get_credits(self) -> str:
        """Check the Commune account balance.

        Use this tool to monitor account credits — for example to ensure
        there is sufficient balance before sending a batch of emails or
        SMS messages.

        Returns:
            str: A formatted string showing the current balance and currency.
            Includes a warning if the balance is below $5.00.
        """
        try:
            result = self._client.credits.get()
            balance = result.get("balance", 0.0)
            currency = result.get("currency", "USD")
            output = f"Balance: ${balance:.2f} {currency}"
            if balance < 5.0:
                output += " — Warning: Low balance!"
            return output
        except Exception as e:
